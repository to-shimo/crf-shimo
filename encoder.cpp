...
namespace CRFPP {
	int max_line_nums = 300000; //default line nums for vector.capacity();
namespace {
...
const CRFPP::Option long_options[] = {
  ...
  {"sentence_line_nums", 'N', "300000", "INT",
   "the number of input lines counted by empty_line(default 300000). it can acclerate the speed of loading data"},
...
int crfpp_learn(const Param &param) {
  ...
  max_line_nums = param.get<int>("sentence_line_nums");
  if(max_line_nums <= 0) {
	  std::cerr << "use default max_line_nums=100000" << std::endl;
  }
