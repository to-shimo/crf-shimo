...
static const CRFPP::Option long_options[] = {
  ...
  {"isTermCat", 'T', 0, 0, "output concatenate term with [S|B|M|E] tagset"},
  {"isWithTermProb", 'P', 0, 0, "output concatenate term's prob"},
  {"isRawCnSentence", 'R', 0, 0, "input raw chinese sentence:not one line by a chinese character"},
  {0, 0, 0, 0, 0}
};
}  // namespace
...
int TaggerImpl::chomp(char* ln) {
	if(!ln) return 0;
	int sz = strlen(ln);
	while((sz>0) && (ln[sz-1]=='\n' || ln[sz-1]=='\r' || ln[sz-1]=='\t' || ln[sz-1]==' ')) {
		ln[--sz] = 0;
	}
	return sz;
}

//read raw chinese sentence
bool TaggerImpl::read_raw_cn(std::istream *is) {
  ...
  //for (;;) {
	...
	char tmp[1024];
	while(idx < sz) {
		int nextn = crfshimo_get_hanzi_len_utf8(line.get(), sz, idx);
		if(nextn <= 0) nextn = 1;
		tmp[0] = 0;
		strncpy(tmp, &line[idx], nextn);
		tmp[nextn] = 0;
		idx += nextn;
		if (!add(tmp)) {
		  return false;
...
bool TaggerImpl::parse_stream(std::istream *is,
        std::ostream *os, bool cat, bool is_with_term_prob, bool is_raw_cn_sent) {
  if(!is_raw_cn_sent) {
	if (!read(is) || !parse()) {
		return false;
	}
  } else {
	if (!read_raw_cn(is) || !parse()) {
		return false;
	}
  }
  if (x_.empty()) {
...
void TaggerImpl::print_concat(bool with_prob) {
  char tmp[20480];
  char tmp1[2048];
  float p = 0.0;
  int num = 0;
  ...
  for(size_t i = 0; i < x_.size(); ++i) {
	...
	const char* c = yname(y(i));
	//std::cout << "c:" << c << std::endl;
	if(strcmp(c, "S") == 0) {
		strcat(tmp, tmp1);
		p = prob(i);
		if(with_prob) os_ << tmp << "|" << p << "\t";
		else os_ << tmp << "\t";
		tmp[0] = 0;
		p = 0.0;
		num = 0;
	} else if(strcmp(c, "B")==0 || strcmp(c, "M")==0 || strcmp(c, "E")==0) {
		strcat(tmp, tmp1);
		num++;
		p += prob(i);
		if(strcmp(c, "E")==0 && ((i+1<x_.size() && strcmp(yname(y(i+1)), "E")!=0) || i+1>=x_.size())) {
			if(with_prob) os_ << tmp << "|" << p/num << "\t";
			else os_ << tmp << "\t";
			tmp[0] = 0;
			p = 0.0;
			num = 0;
...
const char* TaggerImpl::toString2(bool is_with_term_prob) {
  os_.assign("");
  ...
  if (nbest_ >= 1) {
    for (size_t n = 0; n < nbest_; ++n) {
      ...
      os_ << "# " << n << " " << prob() << '\n';
      print_concat(is_with_term_prob);
    }
  } else {
    if (vlevel_ >= 1) {
      //os_ << "# " << prob() << '\n';
...
int crfpp_test(const Param &param) {
  ...
  bool cat = false; //output term after merge(是否拼接term输出)
  if (param.get<bool>("isTermCat")) {
	cat = true;
  }

  bool catProb = false; //output term's prob(是否输出拼接term的概率)
  if (param.get<bool>("isWithTermProb")) {
	catProb = true;
  }

  bool rawCn = false; //input with raw sentence(输入原始中文片段:非单行单字形式)
  if (param.get<bool>("isRawCnSentence")) {
	rawCn = true;
  }
  ...
  for (size_t i = 0; i < rest.size(); ++i) {
    ...
    while (*is) {
      tagger.parse_stream(is.get(), os.get(), cat, catProb, rawCn);
    }
  }
