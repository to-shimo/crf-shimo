...
class TaggerImpl : public Tagger {
 public:
  ...
  bool         parse_stream(std::istream *is, std::ostream *os, bool iscat, bool is_term_with_prob, bool is_raw_cn_sent);
  bool         read_raw_cn(std::istream *is);
  ...
  void print_concat(bool isWithTermProb);
  const char* toString();
  const char* toString2(bool isWithTermProb);
